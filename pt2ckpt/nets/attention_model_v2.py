...
class AttentionStepModel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.is_vrp or self.is_orienteering or self.is_pctsp:
            ...
            node_dim = 2 + self.num_split  # x, y; 前面的序列和，后面的序列和
        ...
        self.project_selected_context = nn.Linear(self.node_size-1, embedding_dim, bias=False)
        self.project_remaining_context = nn.Linear(1, embedding_dim, bias=False)
    ...
    def forward(self, input, ori_input, cost_c_input,return_pi=False):
        """
        ...
        # print(ori_input.size(), input.size(), pi.size())
    ...
    def _inner(self, input, embeddings):
        ...
        i = 0
        selected_dividers = torch.zeros((embeddings.size(0),1,embeddings.size(-1))).to(embeddings.device)
        while True:#not (self.shrink_size is None and state.all_finished()):
            if i >= self.num_split:
                break
            ...
            log_p, mask = self._get_log_p(fixed, state, selected_dividers)
    ...
    def _get_log_p(self, fixed, state, selected_dividers, normalize=True):
        ...
        # print("step_embedding: ",step_embedding.size()) #batch, 1, 128
        # print("context node embedding: ", fixed.context_node_projected.size()) #batch, 1, 128
        # print("embedding: ", fixed.node_embeddings.size()) # batch, nodesize, 128
        # print("visited: ", state.visited.float().size()) # 1表示选择过的，0表示没选择过的 batch,1,nodesize
        # print("remain: ", (self.num_split - state.visited.sum(dim=-1,keepdim=True)).float().size())# batch,1,1
        # 添加一个已经选择节点的累加
        selected_dividers += self.project_step_context(step_embedding)
        # 用一个变量存储已选择divider的全局状态特征
        selected_states = self.project_selected_context(state.visited.float())
        # 用一个变量存储剩余可操作次数
        remain_steps = self.project_remaining_context((self.num_split - state.visited.sum(dim=-1,keepdim=True)).float())
