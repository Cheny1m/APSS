...
def get_cost_e(cluster_info, model_config, parallel_config, amp_config):    
    ...
    # cost_e = [mnp.array(c) for c in cost_e]  # Convert each element in the list to MindSpore Tensor
    # cost_e = ops.stack(cost_e, axis=0)   
...
def predict(config, bs, mbs, cluster_info, model_config, amp_config, oth):
    ...
    if np.all(config == -1):
        ...
        if pp >= (L + 2):
            ...
            return None, None, mnp.array([float("inf")])
