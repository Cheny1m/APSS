...
def compute_in_batches(f, calc_batch_size, *args, n=None):
    ...
    def safe_cat(chunks, axis=0):
        if chunks[0] is None:
            assert all(chunk is None for chunk in chunks)
            return None
        return ops.Concat(axis=axis)(chunks)
    ...
    if isinstance(all_res[0], tuple):
        return tuple(safe_cat(res_chunks, axis=0) for res_chunks in zip(*all_res))
    return safe_cat(all_res, axis=0)