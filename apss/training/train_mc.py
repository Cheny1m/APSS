...
def train_all(model, optimizer, baseline, loss_fn,lr_scheduler, val_dataset, problem, tb_logger, opts):

    def forward_fn(split_x, ori_x, cost_c_x, bl_cost):
        cost, log_likelihood, pi = model(split_x, ori_x, cost_c_x, return_pi=True)
        cost2, log_likelihood2, pi2 = model(split_x, ori_x, cost_c_x, return_pi=True) # 内存增长
    ...
    def train_batch(model, optimizer, baseline, epoch, batch_id, step, batch, tb_logger, opts):
        ...
        (loss,cost,pi,log_likelihood),grads = grad_fn(split_x, ori_x, cost_c_x, bl_cost)# 内存增长
    ...
    # def train_epoch(model, optimizer, baseline, loss_fn,lr_scheduler,epoch, val_dataset, problem, tb_logger, opts):
        ...
        # training_dataset = ds.GeneratorDataset(source=data_generator(training_dataset_), column_names=['data','ori_data','cost_c_data','baseline_cost', 'baseline_pi'],num_parallel_workers=1)
