...
#     ids: ms.Tensor  # Keeps track of original fixed data index of rows

#     # State
#     # first_a: ms.Tensor
#     prev_a: ms.Tensor
#     visited_: ms.Tensor  # Keeps track of nodes that have been visited
#     # lengths: ms.Tensor
#     # cur_coord: ms.Tensor
#     i: ms.Tensor  # Keeps track of step

#     # _fields : ['loc','ids','prev_a','visited_','i']
...
#     #     if name in self._fields:  # _fields is an attribute of NamedTuple which contains all field names
#     #         return StatePP.__getattribute__(self, name)
#     #     raise AttributeError(f"'{type(self).__name__}' StatePP has no attribute '{name}'")
...
# #             lengths = self.lengths + (cur_coord - self.cur_coord).norm(p=2, axis=-1)  # (batch_dim, 1)
...
#     def get_mask(self):
#         return self.visited > 0  # Hacky方式返回bool或uint8取决于MindSpore版本
